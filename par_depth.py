...
def children(constraints, incumbenttotal, decisionvartobranch, newincumb, const_number):
    #print "Decisionvartobranch", constraints, decisionvartobranch, rank
...
def depth_first(to_add, incumbenttotal, (bound, constraints, decisionvartobranch)):
    ...
    if len(res0)>1 and len(res1)>1: #true is both nodes produce more future nodes to pursue. 
        #both res0 and res1 are in the form: [newincumb,(bound, copy.deepcopy(constraints), decisionvar)]
    ...
    elif isValid0 and isValid1: #if both valid solutions are feasible and better than incumbenttotal
...
def master_depth(comm):
    status = MPI.Status()
    size = comm.Get_size() 
    workerq = set(xrange(1,size) ) #set of all workers (slave processors)
    queue = Queue.Queue() #construct  of jobs que
    ...
    while not queue.empty() or len(workerq) < (size-1):
        ...
        while not queue.empty() and len(workerq) > 0:   
            #while not queue.empty() or not workerq.full(): #and nothing running
            
            next = queue.get()
            bound, constraints, decisionvar = next
            ...
            to_send = (incumbenttotal, bound, constraints, decisionvar)
...
#def slave(comm,TIME_COMM_SLAVE, TIME_ALL_SLAVE):
def slave_depth(comm):
    status = MPI.Status()
    rank = comm.Get_rank()
    ...
    while True:
        ...
        incumbenttotal, bound, constraints, decisionvartobranch = data

        to_add = []
        newincumb = depth_first(to_add, incumbenttotal, (bound, constraints, decisionvartobranch))
